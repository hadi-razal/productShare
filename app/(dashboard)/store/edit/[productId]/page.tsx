...
import { getProduct, getStoreById, listProductsByStore, updateProduct, updateStore } from "@/lib/db";
import { deletePublicFiles, uploadPublicFile } from "@/lib/storage";
...
const EditSkeleton = () => (
  <div className="ds-page">
    <div className="grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-6 items-start animate-pulse">
      ...
      <div className="hidden xl:block h-[420px] rounded-2xl bg-gray-100" />
    </div>
  </div>
);
...
const EditProduct = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        ...
        setOwnerId(user.uid);
        const [data, store, products] = await Promise.all([
          getProduct(user.uid, productId as string),
  ...
  const removeImage = (index: number) => {
    const existingCount = productData.images.length;
    if (index < existingCount) {
      ...
      if (removed) setRemovedImageUrls((prev) => [...prev, removed]);
      return;
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      await updateProduct(user.uid, productId as string, updatedData);
      try {
        await deletePublicFiles(removedImageUrls);
      } catch (storageError) {
        console.error("Product updated, but removed images could not be deleted from storage:", storageError);
      }
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-6 items-start">
        ...
        <aside className="xl:sticky xl:top-6 xl:self-start">
          <ProductPreview
            ...
            previewVideo={productData.video || ""}
          />
        </aside>
      </div>
    </div>
